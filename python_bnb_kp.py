...
class bnb(knapsack):
    ...
    def branch_and_bound(self, final_sol):
        ...
        root = struc_sol()
        root.fixed = 0
        self.frac_bound(root, 0)
        current_best = root.copy().val
        self.insert(root)
        currentNode = root

        while (self.QueueSize != 0) and (currentNode.bound > current_best):
            currentNode = self.removeMax()
            currentNode.fixed = currentNode.fixed + 1
            child1 = currentNode.copy()
            child2 = currentNode.copy()
            child1.solution_vec[currentNode.fixed] = True
            child2.solution_vec[currentNode.fixed] = False
            flag = self.frac_bound(child1, currentNode.fixed)
            if (flag):
                if child1.val > current_best:
                    current_best = child1.val
                    self.copy_array(child1.solution_vec, final_sol)
                self.insert(child1)

            flag = self.frac_bound(child2, currentNode.fixed)
            if (flag):
                if child2.val > current_best:
                    current_best = child2.val
                    self.copy_array(child2.solution_vec, final_sol)
                self.insert(child2)
